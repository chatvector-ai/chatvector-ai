...
export default function ArchitecturePage() {
  return (
    <div className="max-w-[720px] mx-auto px-4 py-10">
      ...
      <h1 className="text-3xl font-bold mb-4 text-foreground">Architecture</h1>
      ...
      <p className="text-foreground text-[1rem] leading-[1.8] mb-4">
        The stack is deliberately layered so each concern is isolated. From top
        to bottom:
      </p>
...
└─────────────────────────────────┘`}</pre>
      ...
      <p className="text-foreground text-[1rem] leading-[1.8] mb-6">
        ...
        application code never talks to a specific database driver directly — it
        calls a common interface backed by either <strong>SQLAlchemy</strong>{" "}
        (local / Docker) or <strong>Supabase</strong> (hosted production).
        Swapping backends requires only an environment variable change.
      </p>
      ...
      <p className="text-foreground text-[1rem] leading-[1.8] mb-4">
        ...
        — use explicit retry logic with exponential back-off to handle transient
        failures. This is a blocking dependency for batch ingestion and
        validation features (see{" "}
        <a
    ...
    try:
      ...
      <div className="bg-surface border border-border border-l-[3px] border-l-accent p-4 mb-10">
        <p className="text-foreground text-[1rem] leading-[1.8]">
          ...
          with a background worker pool, token bucket rate limiter, exponential
          backoff with jitter, and a dead-letter queue for failed jobs. A Redis
          replacement is planned for Phase 2 to support higher-throughput batch
          processing at scale.
        </p>
      </div>
      ...
      <p className="text-foreground text-[1rem] leading-[1.8] mb-4">
        Similarity search is handled by <strong>pgvector</strong> running inside
        Postgres — no separate vector database required. Embeddings are stored
        alongside document metadata in the same schema, keeping the operational
        footprint minimal.
      </p>
      <CodeBlock
      ...
      <div className="space-y-3 mb-10">
        {[
          ...
          >
            ...
            <p className="text-foreground text-[0.92rem] leading-[1.7]">
              {body}
            </p>
          </div>
        ))}
